...
Last inspected: **2026-09-19 07:01 UTC**; user timezone: Europe/Moscow.
...
**Stage-2 completed drive validation:** one Tesla Model 3 was spawned at the start of each approved route in synchronous `Town01_Opt`, after reapplying direct hiding to 25 RoadLines objects. The initial two `TrafficManager.set_path` pilots accepted valid stored coordinates but departed route 1 at a later branch, so they were finalized as incomplete diagnostics. The corrected `TrafficManager.set_route` implementation derives one `Left`/`Right`/`Straight` instruction per contiguous map-junction traversal and records the full 2 m chain independently for trajectory projection. The final run `20260919T065802Z-tm-autopilot-approved-routes-106f97` completed all five routes, with finish distance 7.20–7.93 m (criterion ≤8 m), maximum route deviation 0.997–1.247 m (limit ≤15 m), zero collision events, no timeout/stuck result, and a stopped vehicle after each finish. Its 19 signed files passed local entry-by-entry verification. This meets Stage-2 items 5–6; no cameras or dataset imagery were created. The CARLA container is stopped. The tested search found no connected Driving-waypoint candidate through the central narrow visual bridge; intermediate rejected/failed runs are retained as incomplete.
...
| 2. Routes | DONE | All five user-approved routes were driven by a spawned TM/autopilot vehicle on Town01_Opt. `20260919T065802Z-tm-autopilot-approved-routes-106f97` passed completion, deviation, collision, timeout/stuck, and post-finish-stop checks; two set_path diagnostics are retained. |
...
- The 12,180,606-byte candidate run `20260919T055800Z-route-candidates-adaptive-a2` is locally manifest-verified; its first three geometries are preserved in the final approved set. The 12,442,715-byte revision run `20260919T062007Z-route-revision-ca00ee` contains all five user-approved geometries and is the authoritative source for subsequent Stage-2 driving. Its manifest SHA-256 is `db2018bf734ce412e53d8b396d7053ae5f879d76ad95571b7ede42e8f6b63936`. Ten intermediate route-revision runs are retained as incomplete design/validation evidence. None is copied externally.
- Stage-2 driving evidence is local-only: the full successful five-drive run `20260919T065802Z-tm-autopilot-approved-routes-106f97` is 2,258,589 bytes, has manifest SHA-256 `607b8b527c62aef0565978b512ab80182c07c380d8ab26f9638bfe29122fbe86`, and passed local verification of 19 signed files. Two failed `set_path` diagnostics and two successful one-route `set_route` pilots are also finalized and registered. None is copied externally.
