...
Last inspected: **2026-09-17 19:45 UTC**; user timezone: Europe/Moscow.
...
| 2. Routes | IN_PROGRESS | Five connected 60-waypoint routes, complete 2 m network sample, route files, and temporary DebugHelper views passed local validation in `20260917T194300Z-town01-opt-routes-final-c5b492`; valid actor spawns, Traffic Manager paths, and actual completion/stuck/collision checks remain |
...
- The locally manifest-verified 12,884,530-byte Stage-2 route-selection run `20260917T194300Z-town01-opt-routes-final-c5b492` is registered with `backup_status=not_copied`. It contains `network_waypoints.json`, `routes.json`, five route files, and seven DebugHelper top-down PNGs. The two earlier same-session visualization iterations (`20260917T193903Z-town01-opt-routes-b168cc` and `20260917T194057Z-town01-opt-routes-readable-d601e3`) are retained locally; the final run is the adopted evidence because it separates full-network and per-route views.
