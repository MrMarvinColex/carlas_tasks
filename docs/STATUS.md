...
Last inspected: **2026-09-19 06:20 UTC**; user timezone: Europe/Moscow.
...
**Stage-2 replacement review:** the user approved candidate geometries 1–3 from `20260919T055800Z-route-candidates-adaptive-a2`; their waypoint identities were preserved exactly. Run `20260919T062007Z-route-revision-ca00ee` revises route 4 to four turns (508.150 m; 119 adaptive points from a 252-point dense chain) and route 5 to a complete outer automobile-bridge crossing followed by a turn (495.316 m; 101 adaptive points from a 244-point dense chain). All dense edges passed direct `Waypoint.next(2.0)` validation. RoadLines hiding was reapplied, saved previews use `draw_line` only, and the final manifest passed entry-by-entry verification. Routes 4–5 still await the user's visual approval, so the older five-route set remains the operational fallback until the replacement set is approved as a whole. No ego actor or Traffic Manager drive has been run. The tested search found no connected Driving-waypoint candidate through the central narrow visual bridge; intermediate rejected/failed runs are retained as incomplete. No CARLA container is running.

**Next action:** obtain the user's visual approval or requested edits for revised routes 4–5 in `20260919T062007Z-route-revision-ca00ee`. After approval, promote the complete replacement set and implement the Traffic Manager driving half of Stage 2 against those five routes. The supplementary Town10HD_Opt reference-view probe remains independent and non-blocking.
...
| 2. Routes | IN_PROGRESS | Replacement routes 1–3 are user-approved; revised 4–5 are locally validated and await visual approval in `20260919T062007Z-route-revision-ca00ee`. Valid actor spawns, Traffic Manager paths, and actual completion/stuck/collision checks remain. |
...
- The 12,180,606-byte candidate run `20260919T055800Z-route-candidates-adaptive-a2` is locally manifest-verified; its first three geometries are user-approved. The final 12,442,715-byte revision run `20260919T062007Z-route-revision-ca00ee` preserves those three and proposes revised routes 4–5. Its manifest SHA-256 is `db2018bf734ce412e53d8b396d7053ae5f879d76ad95571b7ede42e8f6b63936`. Ten intermediate route-revision runs are retained as incomplete design/validation evidence. None is copied externally.
