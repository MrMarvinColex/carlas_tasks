...
Last inspected: **2026-09-19 06:40 UTC**; user timezone: Europe/Moscow.
...
**Stage-2 approved route set:** the user approved all five adaptive geometries on 2026-09-19. Routes 1–3 originate from `20260919T055800Z-route-candidates-adaptive-a2` and their waypoint identities are preserved exactly in the final run. Run `20260919T062007Z-route-revision-ca00ee` supplies the approved route 4 with four turns (508.150 m; 119 adaptive points from a 252-point dense chain) and route 5 with a complete outer automobile-bridge crossing followed by a turn (495.316 m; 101 adaptive points from a 244-point dense chain). All dense edges passed direct `Waypoint.next(2.0)` validation. RoadLines hiding was reapplied, saved previews use `draw_line` only, and the final manifest passed entry-by-entry verification. This five-route set now supersedes the older 60-point operational fallback for subsequent Stage-2 work. No ego actor or Traffic Manager drive has been run. The tested search found no connected Driving-waypoint candidate through the central narrow visual bridge; intermediate rejected/failed runs are retained as incomplete. No CARLA container is running.

**Next action:** implement a one-route Traffic Manager/autopilot pilot against the approved route files in `20260919T062007Z-route-revision-ca00ee`, including valid spawning, path submission, completion radius, timeout, stuck detection, collision logging, and post-finish stopping. After the pilot passes, run and validate all five routes. The supplementary Town10HD_Opt reference-view probe remains independent and non-blocking.
...
| 2. Routes | IN_PROGRESS | All five adaptive route geometries are user-approved and locally validated in `20260919T062007Z-route-revision-ca00ee`. Valid actor spawns, Traffic Manager paths, and actual completion/stuck/collision checks remain. |
