...
**ARTIFACT-CONFIRMED (2026-09-24, Stage 7 initial audit):** the installed `carlasim/carla:0.9.16` image is pinned to digest `sha256:aaf1df22702780ece072069e23d03c4879b002ae028c79744b09c4c7ddbae953`; no CARLA server container was running. A temporary container with `--rm --network none` searched `/workspace/CarlaUE4/Content` for animal-name candidates. Its only animal-like filename was `DogHouse`; the packaged Blueprint roots expose Props, Vehicles and Walkers but no Animal directory. Together with the earlier live catalogue result (214 IDs; zero matching animal candidate IDs), no usable built-in animal actor or clearly named animal mesh has been observed. This is evidence of absence in the exposed catalogue and searched filenames, not proof that every arbitrarily named hidden mesh is absent. The official 0.9.16 prop-authoring procedure requires an Unreal Editor import and a newly made CARLA package for a raw external asset, so an FBX/GLB download cannot satisfy U02 by Python API alone (S14). A possible third-party cooked-boar pack was rejected before download: its own untagged pre-release README requires exact CARLA 0.9.15, not 0.9.16 (S23).

**ARTIFACT-CONFIRMED (2026-09-24, Stage 7):** AnimaSim v0.2.1 archive `animasim-carla-0.2.1.tar.gz` matched its published 243,116,126-byte size and SHA-256 `4462ef4099a7057764c7d9fdf5788efdf0ddd6daad156e55440c55a8a834e071`. It was installed by the documented consumer-side `ImportAssets.sh` only in derived image `carlasim/carla:0.9.16-animasim-v0.2.1` (`sha256:2d6fb34a8e78b159b225ff206473854f040c3e8612dba3c6e22ae85d52f5b9b9`); its label preserves the unchanged base digest. A live 0.9.16 server returned all twelve documented static animal blueprints. The AnimaSim code is MIT and its declared Quaternius Ultimate Animated Animal Pack source is CC0 (S24).

**ARTIFACT-CONFIRMED (2026-09-24, `20260924T135331Z-animasim-deer-front-probe-grounded-d91894`):** `static.prop.deer` visibly spawned on `Town01_Opt` route 1, appeared in both front-centre RGB and CARLA CityScapes preview, and added 1,187–1,544 raw semantic pixels of runtime enum `carla.CityObjectLabel.Dynamic == 21` above its baseline in each of 14 aligned 2 Hz samples. The Python `Actor.semantic_tags` list was empty for this imported prop; raw image evidence plus the API enum, not a guessed actor attribute, is the semantic result. With z offset 0, the deer bounding-box minimum and route-ground height were both 0.0 m; its extents were (0.2613, 0.7870, 0.7650) m. The saved 139-command trajectory completed the configured 12 m lateral path at 2 m/s: maximum requested 0.05 s movement was 0.100006 m, below the 0.125 m bound, and endpoint error was 0.0000076 m. A directly driven Tesla collision sensor registered `static.prop.deer`; retain that as collision-geometry evidence only.

**ARTIFACT-CONFIRMED (2026-09-24, Stage-7 API comparison):** SceneSpec v1.1 contains only `animal.type`, route-relative anchor, named lateral trajectory, speed and start time; every value but seed is fixed to the grounded probe. All three one-attempt API replies in `20260924T140505Z-api-verified-deer-crossing-e6313d` passed the strict parser/resolver and matched outside seed. Each associated single-camera replay passed the same probe: GPT `20260924T140639Z-api-gpt-deer-crossing-9284ba` had Dynamic delta 1,187–1,544 pixels; Qwen-27B `...-api-qwen27-deer-crossing-4146e3` 1,194–1,543; Qwen-8B `...-api-qwen8-deer-crossing-9490d8` 1,185–1,544. Their non-seed scene content was identical; the Qwen response bytes/specifications also matched. `stage7_comparison.json` records API/local timings, usage, scene digests and replay metrics. This is a compliance test with a single supported scene, not a ranking of generative ability.

Kinematic motion without leg animation is acceptable for the stated visual effect, but it is repeated transform placement rather than physical locomotion. It does not prove collision behaviour between measurements or a Traffic Manager reaction. Do not secretly apply ego braking and report it as natural reaction; if another controller is used by agreement, disclose it.
...
| S23 | [OOD-PerceptionBench repository](https://github.com/farrosalferro/OODPerceptionBench) | Investigated only as a possible cooked boar asset; its pre-release README pins the content pack to exact CARLA 0.9.15, so it is not a 0.9.16 candidate; accessed 2026-09-24 |
| S24 | [AnimaSim v0.2.1 release](https://github.com/danwahl/animasim/releases/tag/v0.2.1), [versioned CARLA instructions](https://github.com/danwahl/animasim/tree/v0.2.1/platforms/carla), and [Quaternius pack](https://quaternius.com/packs/ultimateanimatedanimals.html) | Source-verified candidate: cooked 0.9.16 static-animal package, installation procedure and declared MIT/CC0 terms; accessed 2026-09-24 |
